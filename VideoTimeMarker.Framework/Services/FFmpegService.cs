...
namespace VideoTimeMarker.Framework.Services
{
    ...
    public class FFmpegService : IFFmpegService
    {
        ...
        public async Task<int> AddDynamicTimeWatermark(string inputFile, string outputFile, DateTime startTime, TimeSpan duration, int fontSize = 45, int x = 20, int y = 20)
        {
            ...
            outputFile = Path.Combine(directory ?? string.Empty, $"{fileNameWithoutExt}_{timeStamp}{extension}");
        ...
        public async Task<int> CropVideo(string inputFile, string outputFile, int width, int height, int x, int y)
        {
            ...
            outputFile = Path.Combine(directory ?? string.Empty, $"{fileNameWithoutExt}_crop_{timeStamp}{extension}");
        ...
        public async Task<int> CropAndAddWatermark(string inputFile, string outputFile, int width, int height, int x, int y, DateTime startTime, TimeSpan duration, int fontSize = 40, int watermarkX = 18, int watermarkY = 18)
        {
            ...
            var extension = fileInfo.Extension;
            var timeStamp = startTime.ToString("yyyyMMdd_HHmmss");
            outputFile = Path.Combine(directory ?? string.Empty, $"{fileNameWithoutExt}_crop_watermark_{timeStamp}{extension}");
        ...
        /// 获取视频信息（宽度和高度）
        /// </summary>
        private async Task<VideoSize> GetVideoInfoAsync(string inputFile)
        {
            var process = new Process
            {
                StartInfo = new ProcessStartInfo
            ...
            try
            {
                ...
                // 解析FFmpeg输出以获取视频尺寸
                var videoSizeMatch = Regex.Match(output, @"Stream.*Video.*\s(\d+)x(\d+)");
                if (videoSizeMatch.Success)
                {
                    ...
                    return new VideoSize { Width = width, Height = height, IsValid = true };
                }

                return new VideoSize { IsValid = false };
            }
            catch (Exception ex)
            {
